...
const DocxViewer = ({
  ...
  useEffect(() => {
    if (contentRef.current) {
      // Initial sync or reset if empty
...
function App() {
  ...
  const renderToolbar = () => {
        ...
        <div className="flex items-center gap-2 h-full">
          <div className="flex items-center gap-1 pr-4 border-r border-slate-700">
            <ToolButton
              icon={Wand2}
              label="Summarize"
              onClick={() => handleAiSubmit("Summarize this document")}
            />
            <ToolButton
              icon={Languages}
              label="Translate"
              onClick={() =>
                handleAiSubmit("Translate this document to Spanish")
              }
            />
            <ToolButton
              icon={ScanLine}
              label="Fix Grammar"
              onClick={() =>
                handleAiSubmit("Check for grammar errors in this text")
              }
            />
          </div>
          <div className="ml-auto flex items-center gap-2">
            <span className="text-xs text-slate-400">
              ...
              <strong
                className={
                  aiMode === "cloud" ? "text-indigo-400" : "text-emerald-400"
                }
              >
                {aiMode.toUpperCase()}
              </strong>
    ...
    return (
      <div className="flex items-center h-full text-slate-500 text-sm px-4">
        Tools for {activeToolGroup} coming soon...
      </div>
    );
  };
      ...
      >
        ...
        <div className="w-20 bg-slate-950 flex flex-col items-center py-4 border-r border-slate-800 z-20 shadow-xl">
          ...
          <nav className="flex flex-col gap-6 w-full items-center">
            <SidebarBtn
              icon={FileText}
              active={activeToolGroup === "Home"}
              onClick={() => setActiveToolGroup("Home")}
            />
            <SidebarBtn
              icon={ScanLine}
              active={activeToolGroup === "Convert"}
              onClick={() => setActiveToolGroup("Convert")}
            />
            <SidebarBtn
              icon={Layers}
              active={activeToolGroup === "Merge"}
              onClick={() => setActiveToolGroup("Merge")}
            />
            <SidebarBtn
              icon={Bot}
              active={activeToolGroup === "AI Assistant"}
              onClick={() => setActiveToolGroup("AI Assistant")}
            />
            <div className="h-px w-8 bg-slate-800 mx-auto"></div>
            <SidebarBtn
              icon={Settings}
              active={activeToolGroup === "Settings"}
              onClick={() => setActiveToolGroup("Settings")}
            />
          </nav>
        </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="h-28 bg-slate-800 border-b border-slate-700 shadow-lg flex flex-col z-10">
            <div className="flex px-4 text-xs font-medium gap-6 pt-2 text-slate-400 border-b border-slate-700/50">
              {["Home", "Edit", "View", "AI Assistant", "Convert"].map(
                (group) => (
                  <button
                    key={group}
              ...
              <div className="w-80 bg-slate-950 border-l border-slate-800 flex flex-col shadow-2xl z-30 transition-all duration-300">
                <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/90 backdrop-blur-sm">
                  <span className="font-semibold flex items-center gap-2 text-indigo-100">
                    <Bot size={18} className="text-indigo-500" /> Omnis Copilot
                  </span>
                ...
                <div className="px-4 py-3 bg-slate-900 border-b border-slate-800">
                  <div className="flex bg-slate-950 p-1 rounded-lg border border-slate-800">
                    <button
                      ...
                      className={`flex-1 text-[10px] uppercase font-bold tracking-wider py-1.5 rounded-md flex items-center justify-center gap-1 transition-all ${aiMode === "cloud" ? "bg-indigo-600 text-white shadow-sm" : "text-slate-500 hover:text-slate-300"}`}
                    >
                      <Wand2 size={12} /> Cloud
                    </button>
                    <button
                      onClick={() => setAiMode("local")}
                      className={`flex-1 text-[10px] uppercase font-bold tracking-wider py-1.5 rounded-md flex items-center justify-center gap-1 transition-all ${aiMode === "local" ? "bg-emerald-600 text-white shadow-sm" : "text-slate-500 hover:text-slate-300"}`}
                    >
                ...
                <div className="flex-1 p-4 space-y-4 overflow-y-auto custom-scrollbar bg-slate-900/50">
                  {chatHistory.map((msg) => (
                    ...
                    >
                      <div
                        className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center text-xs font-bold ${msg.role === "ai" ? "bg-indigo-600 text-white" : "bg-slate-700 text-slate-300"}`}
                      >
                        {msg.role === "ai" ? "O" : "Me"}
                      </div>
                      <div
                        className={`rounded-2xl p-3 text-sm leading-relaxed shadow-sm max-w-[85%] ${msg.role === "ai" ? "bg-slate-800 text-slate-300 rounded-tl-none" : "bg-indigo-600 text-white rounded-tr-none"}`}
                      >
                        {msg.role === "ai" ? (
                  ...
                  {isAiThinking && (
                    <div className="flex gap-3">
                      <div className="w-8 h-8 rounded-full bg-indigo-600 shrink-0 flex items-center justify-center text-white text-xs font-bold">
                        O
                      </div>
                  ...
                  <div className="relative group">
                    <input
                      ...
                      placeholder={
                        activeFile
                          ? "Ask about this file..."
                          : "Open a file to ask questions..."
                      }
                      disabled={!activeFile || isAiThinking}
                      value={chatInput}
                      onChange={(e) => setChatInput(e.target.value)}
                      onKeyDown={(e) =>
                        e.key === "Enter" && handleAiSubmit(chatInput)
                      }
                    ...
                    <button
                      ...
                      disabled={
                        !chatInput.trim() || !activeFile || isAiThinking
                      }
...
const SidebarBtn = ({
  ...
  <button
    onClick={onClick}
    className={`p-3 rounded-xl transition-all ${active ? "bg-slate-800 text-indigo-400 shadow-lg shadow-indigo-900/20" : "text-slate-400 hover:bg-slate-800 hover:text-white"}`}
  >
