...
// Fix: Use require() to bypass missing type definition error for html-to-docx
...
const analyzeText = (
  ...
  if (
    p.includes("most used") ||
    p.includes("frequency") ||
    p.includes("common word")
  ) {
    ...
    const prefix =
      mode === "cloud" ? "<b>Cloud Frequency Analysis</b><br>" : "";
  ...
  if (
    ...
    const title =
      mode === "cloud"
        ? "<b>Cloud Executive Summary:</b>"
        : "<b>Local Summary Analysis:</b>";
    const footer =
      mode === "cloud"
        ? "<i>(Generated via Cloud Neural Engine)</i>"
        : "<i>(Generated locally by extracting key structural sentences)</i>";
...
ipcMain.handle("ai:generate", async (_, { prompt, context, mode }) => {
  ...
  await new Promise((resolve) =>
    setTimeout(resolve, mode === "cloud" ? 1500 : 500)
  );
